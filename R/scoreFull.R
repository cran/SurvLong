#' Score calculation for full kernel method                                     #
#'
#' @noRd
...
scoreFull <- function(beta, 
  ...
  for (i in 1L:n) {
    ...
    # Keep only those covariates for this patient
    use <- Zid == X[i, 1L]
    ...
    s2 <- matrix(colSums(outerOnce[ZptIDs, , drop = FALSE] * prod[, 1L]),
                 nrow = p, ncol = p)
